...
(mf/defc typography-entry
  [{:keys [typography show-deattach? on-change on-deattach]}]
  (let [open? (mf/use-state false)
        ...
        (fn [event]
          (let [attrs (merge
                       {:typography-ref-file (:file-id typography)
        ...
        (fn [event]
          (run! #(st/emit! (dwt/update-text-attrs {:id % :editor nil :attrs {:typography-ref-file nil
                                                                             :typography-ref-id nil}}))
                selected))]
    [:*
     [:div.element-set-options-group.typography-entry
      [:div.typography-selection-wrapper
       {:on-click handle-typography-selection}
       ...
       (when show-deattach?
         [:div.element-set-actions-button
          {:on-mouse-enter #(reset! hover-deattach true)
           ...
           :on-click handle-deattach-typography}
